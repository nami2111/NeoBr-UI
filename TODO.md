...
- [x] **P5-D2 (polish)** Landing micro-entry: `.enter` CSS class (0.3s ease-out rise 8px + fade, fill-mode forwards) added to `apps/docs/src/routes/+page.svelte` scoped style, with `prefers-reduced-motion` guard (opacity 1 + animation none). Applied to hero demo panel + metrics (3) + features (3), `animation-delay: i*60ms` per item. Docs-only, library untouched. Docs build rc=0 (only error-*.js filename matches), docs svelte-check 0/0
- [x] **P5-E (docs)** Tokens page: new `apps/docs/src/routes/docs/tokens/+page.svelte` — color swatches (incl. border/input aliases), radius table, depth (shadow-brutalist(-hover), lift/press vars), motion tokens (JS TRANSITION_* + CSS animate-*), z-index scale, shadcn-compat aliases section, @utility helpers. Sidebar entry added. Docs build rc=0 + docs svelte-check 0/0
